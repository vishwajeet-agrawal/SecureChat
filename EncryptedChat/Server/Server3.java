...
public class Server3{
	...
	public static void main(String[] args){
		try{
			new Server3(Integer.parseInt(args[0]),Integer.parseInt(args[1]));
		}
		catch(Exception e){
	...
	private ConcurrentHashMap<String,String> users_pk = new ConcurrentHashMap<>();
	...
	Server3(int port_send, int port_receive){
			...
			while(true){
				...
				if (messg.read()=='\n'){
					if (s1.length()<=16){
						...
						else if (Server3.checkUsernameWellFormed(usr)){
					...
					String usr_to_receive = s1.substring(5);
						...
						if (s1.substring(0,16).equals("Content-length: ")){
							// if (messg.read()=='\n'){
								...
								String hash_msg = messg.readLine();
								...
								// now find the user to send the message to and send him the message
									...
									receive_end.mesg.newLine();
									receive_end.mesg.write("PK ");
									receive_end.mesg.write(users_pk.get(username));
									receive_end.mesg.newLine();
									receive_end.mesg.write(hash_msg);
									receive_end.mesg.newLine();
									// receive_end.mesg.newLine();
						...
						if (s1.length()<=16){
							...
							if (pk1.length()<=4){
								...
								if (receive_users.containsKey(usr)){
									...
									if (send_users.containsKey(usr)){
										users.add(usr);
										users_pk.put(usr,this.publicKey);
									}
									username = usr;
									//success 
